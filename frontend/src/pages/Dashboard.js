...
function Dashboard({ 
  ...
  const handleManualRefresh = async () => {
    if (onRefreshData && !refreshing) {
      setRefreshing(true);
      await onRefreshData();
      setTimeout(() => setRefreshing(false), 1000); // Petit délai pour l'UX
    }
  };
    ...
    <div className="dashboard-container">
      <div className="dashboard-chat-central-area">
        <div className="dashboard-chat-placeholder">
          ...
          {/* ✅ Bouton de rafraîchissement manuel si les données ne se chargent pas */}
          {!loading && (rooms.length === 0 || allUsers.length === 0) && (
            <button 
              onClick={handleManualRefresh}
              disabled={refreshing}
              className="dashboard-refresh-button"
            >
              <RefreshCw className={`dashboard-refresh-icon ${refreshing ? 'spinning' : ''}`} />
